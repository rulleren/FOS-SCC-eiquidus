...
module.exports = {
  ...
  get_pow_extraction: function(limit, cb) {
    Tx.aggregate([
      { $match: {
        $or: [
          { "vin": { $size: 0 } },
      ...
      { $unwind: "$vout" },
      { $match: { "vout.addresses": { $nin: ["coinbase", "unknown_address", null, ""] } } },
  ...
  get_mn_reward_distribution: function(limit, cb) {
    Masternode.find({ last_paid_block: { $gt: 0 } })
      .sort({ last_paid_block: -1 })
      .limit(limit)
      .then((masternodes) => {
        return cb(masternodes || []);
      }).catch((err) => {
