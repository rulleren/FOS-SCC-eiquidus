...
module.exports = {
        ...
        txs.forEach(tx => {
          if (tx.vout) {
            tx.vout.forEach(out => {
              ...
              if (addr && addr !== 'coinbase' && addr !== 'unknown_address' && addr !== '') {
  ...
  get_mn_reward_distribution: function(block_count, result_limit, cb) {
    Tx.findOne({}).sort({ blockindex: -1 }).select('blockindex').lean().then((latest) => {
      if (!latest)
        return cb([], 0);

      const min_block = Math.max(0, latest.blockindex - block_count);

      // Find coinstake transactions (non-coinbase, txIndex=1 = first tx after empty PoS coinbase)
      // For each coinstake, the MN payment is the vout address that is NOT the staker's own address
      Tx.find({
        blockindex: { $gte: min_block },
        txIndex: 1,
        $nor: [
          { "vin": { $size: 0 } },
          { "vin.0.addresses": "coinbase" }
      ...
      .then((txs) => {
        ...
        const counts = {};
        txs.forEach(tx => {
          const vin_set = new Set((tx.vin || []).map(v => {
            const a = Array.isArray(v.addresses) ? v.addresses[0] : v.addresses;
            return a;
          }).filter(a => a && a !== 'coinbase'));

          (tx.vout || []).forEach(out => {
            const addr = Array.isArray(out.addresses) ? out.addresses[0] : out.addresses;
            if (addr && addr !== 'coinbase' && addr !== 'unknown_address' && addr !== '' && !vin_set.has(addr)) {
              counts[addr] = (counts[addr] || 0) + 1;
            }
          });
        });
        ...
        const sorted = Object.entries(counts)
          .map(([addr, count]) => ({ _id: addr, count, claim_name: '' }))
          .sort((a, b) => b.count - a.count)
          .slice(0, result_limit);
        ...
        // Look up claim names from Masternode collection and ClaimAddress collection
        const addresses = sorted.map(item => item._id);
        Masternode.find({ addr: { $in: addresses } }).select('addr claim_name').lean().then((mns) => {
          const mn_claim_map = {};
          mns.forEach(m => {
          ...
          ClaimAddress.find({ a_id: { $in: addresses } }).select('a_id claim_name').lean().then((claims) => {
            claims.forEach(c => {
              if (!mn_claim_map[c.a_id] && c.claim_name && c.claim_name !== '')
                mn_claim_map[c.a_id] = c.claim_name;
            });

            sorted.forEach(item => {
              item.claim_name = mn_claim_map[item._id] || '';
            });

            const total = sorted.reduce((sum, item) => sum + item.count, 0);
            return cb(sorted, total);
          }).catch(() => {
            sorted.forEach(item => { item.claim_name = mn_claim_map[item._id] || ''; });
            const total = sorted.reduce((sum, item) => sum + item.count, 0);
            return cb(sorted, total);
          });
        }).catch(() => {
