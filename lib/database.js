...
module.exports = {
  ...
  get_mn_reward_distribution: function(block_count, result_limit, cb) {
    // In SCC (PoW + Masternodes), each block has two coinbase-type transactions:
    //   txIndex=0: miner reward (counted by get_pow_extraction)
    //   txIndex=1: masternode payment (separate coinbase tx, vin.addresses='coinbase')
    // We scan the last block_count txIndex=1 coinbase txs to get MN reward counts.
    Tx.find({
      txIndex: 1,
      "vin.0.addresses": "coinbase"
    })
