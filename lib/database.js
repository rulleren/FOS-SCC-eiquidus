...
module.exports = {
  ...
  get_address_txs_ajax: function(hash, start, length, cb) {
    AddressTx.find({a_id: hash}).countDocuments().then((totalCount) => {
      // First, get ALL transactions for this address sorted chronologically (oldest first)
      AddressTx.find({a_id: hash})
        .sort({blockindex: 1, txIndex: 1})
        .exec()
        .then((all_address_txs) => {
          // Calculate running balances in chronological order
          let running_balance = 0;
          const txs_with_balance = [];

          for (let i = 0; i < all_address_txs.length; i++) {
            running_balance += all_address_txs[i].amount;
            txs_with_balance.push({
              txid: all_address_txs[i].txid,
              blockindex: all_address_txs[i].blockindex,
              amount: all_address_txs[i].amount,
              balance: running_balance
            });
          }

          // Now reverse for display (newest first) and apply pagination
          txs_with_balance.reverse();

          const paginated_txs = txs_with_balance.slice(Number(start), Number(start) + Number(length));

          // Look up full transaction details
          let txs = [];
          async.eachSeries(paginated_txs, function(current_addresstx, loop) {
            find_tx(current_addresstx.txid, function(tx) {
              if (tx) {
                // set the balance for this tx
                tx.balance = current_addresstx.balance;
  ...
  get_pow_extraction: function(block_count, result_limit, cb) {
    const coinbase_match = {
      $or: [
        { "vin": { $size: 0 } },
        { "vin.0.addresses": "coinbase" }
      ],
      "vout.0": { $exists: true }
    };

    Tx.find(coinbase_match)
      .sort({ blockindex: -1 })
      .limit(block_count)
      .select('vout')
      .lean()
      .then((txs) => {
        ...
        Masternode.find({}).select('addr').lean().then((masternodes) => {
          const mn_addr_set = new Set(masternodes.map(m => m.addr));
          const counts = {};
          txs.forEach(tx => {
            if (tx.vout) {
              const miner_addr = tx.vout
                .map(out => Array.isArray(out.addresses) ? out.addresses[0] : out.addresses)
                .find(addr => addr && addr !== 'coinbase' && addr !== 'unknown_address' && addr !== '' && !mn_addr_set.has(addr));
              if (miner_addr) {
                counts[miner_addr] = (counts[miner_addr] || 0) + 1;
              }
            }
          });

          const sorted = Object.entries(counts)
            .map(([addr, count]) => ({ _id: addr, count }))
            .sort((a, b) => b.count - a.count)
            .slice(0, result_limit);

          return cb(sorted, txs.length);
        }).catch((err) => {
          console.log(err);
          return cb([], 0);
        });
  ...
  get_mn_reward_distribution: function(block_count, result_limit, cb) {
    // In SCC (DASH-fork), the masternode payment is a vout within the coinbase tx,
    // not a separate transaction. We identify MN vouts by cross-referencing coinbase
    // vout addresses against the Masternode collection.
    Masternode.find({}).select('addr claim_name').lean().then((masternodes) => {
      ...
      masternodes.forEach(m => {
        if (m.claim_name && m.claim_name !== '')
          mn_claim_map[m.addr] = m.claim_name;
      });

      const coinbase_match = {
        $or: [{ 'vin': { $size: 0 } }, { 'vin.0.addresses': 'coinbase' }],
        'vout.0': { $exists: true }
      };

      Tx.find(coinbase_match).sort({ blockindex: -1 }).limit(block_count).select('vout').lean()
        .then((txs) => {
          if (!txs || txs.length === 0)
            return cb([], 0);
          ...
          const counts = {};
          txs.forEach(tx => {
            const mn_vout = (tx.vout || []).find(out => {
              const addr = Array.isArray(out.addresses) ? out.addresses[0] : out.addresses;
              return addr && mn_addr_set.has(addr);
            });
            if (mn_vout) {
              const addr = Array.isArray(mn_vout.addresses) ? mn_vout.addresses[0] : mn_vout.addresses;
              counts[addr] = (counts[addr] || 0) + 1;
            }
          });


          const all_entries = Object.entries(counts)
            .map(([addr, count]) => ({ _id: addr, count, claim_name: mn_claim_map[addr] || '' }))
            .sort((a, b) => b.count - a.count);
          const total_all = all_entries.reduce((sum, item) => sum + item.count, 0);
          const sorted = all_entries.slice(0, result_limit);

          if (sorted.length === 0)
            return cb([], 0);

          const addrs_without_claim = sorted.filter(item => !item.claim_name).map(item => item._id);

          if (addrs_without_claim.length === 0) {
            return cb(sorted, total_all);
          }

          ClaimAddress.find({ a_id: { $in: addrs_without_claim } }).select('a_id claim_name').lean()
            .then((claims) => {
              claims.forEach(c => {
                const item = sorted.find(s => s._id === c.a_id);
                if (item && !item.claim_name && c.claim_name && c.claim_name !== '')
                  item.claim_name = c.claim_name;
              });
              return cb(sorted, total_all);
            })
            .catch(() => {
