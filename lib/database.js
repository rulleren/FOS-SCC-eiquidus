...
module.exports = {
  ...
  get_mn_reward_distribution: function(block_count, result_limit, cb) {
    Masternode.find({}).select('addr claim_name').lean().then((masternodes) => {
      ...
      const mn_addresses = masternodes.map(m => m.addr).filter(a => a && a !== '');
      if (mn_addresses.length === 0)
        return cb([], 0);
      ...
      Tx.findOne({}).sort({ blockindex: -1 }).select('blockindex').lean().then((latest) => {
        ...
        Tx.aggregate([
          { $match: { blockindex: { $gte: min_block } } },
          { $addFields: {
            vin_addresses: { $map: { input: "$vin", as: "v", in: "$$v.addresses" } }
          }},
          { $unwind: "$vout" },
          { $match: {
            "vout.addresses": { $in: mn_addresses },
            $expr: { $not: { $in: ["$vout.addresses", "$vin_addresses"] } }
          }},
          { $group: { _id: "$vout.addresses", count: { $sum: 1 } } },
          { $sort: { count: -1 } },
          { $limit: result_limit }
        ]).then((result) => {
          result = result.map(item => ({
            ...item,
            claim_name: mn_claim_map[item._id] || ''
          }));
