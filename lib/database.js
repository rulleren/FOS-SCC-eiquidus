...
module.exports = {
  ...
  get_mn_reward_distribution: function(block_count, result_limit, cb) {
    // Find the last block_count coinstake transactions (PoS block rewards).
    // We do NOT restrict by block height window — SCC is currently ~90% PoW, so a
    // 1000-block window only contains ~100 PoS blocks. Instead we scan back as far
    // as needed to collect block_count coinstake txs.
    Tx.find({
      txIndex: 1,
      $nor: [
        { "vin": { $size: 0 } },
