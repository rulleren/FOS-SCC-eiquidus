...
function update_addresses(addresses, blockheight, txid, txIndex, cb) { // ADD txIndex parameter
        ...
        updateOne: {
          ...
          update: {
            $setOnInsert: { 
              ...
              txid: txid,
              txIndex: txIndex // ADD THIS - store txIndex in AddressTx
            },
...
module.exports = {
  save_tx: function(txid, blockheight, block, txIndex, cb) { // ADD txIndex parameter
    try {
          ...
          lib.prepare_vin(tx, function(vin, tx_type_vin) {
            lib.prepare_vout(tx.vout, txid, vin, ((!settings.blockchain_specific.zksnarks.enabled || typeof tx.vjoinsplit === 'undefined' || tx.vjoinsplit == null) ? [] : tx.vjoinsplit), function(vout, nvin, tx_type_vout) {
              ...
              if (vout != null) {
                ...
                // save the addresses to the database - PASS txIndex parameter
                update_addresses(addressBatch, blockheight, txid, txIndex, function(err) {
                  if (err)
                    ...
                    // return the transaction data - ADD txIndex field
                    return cb(null, vout.length > 0, new Tx({
                      ...
                      blockindex: blockheight,
                      txIndex: txIndex, // ADD THIS LINE - store transaction index
                ...
                lib.get_block(blockhash, function(block) {
                  if (block) {
                    ...
                    async.eachLimit(block.tx, parallel_tasks, function(txid, next_tx) {
                      // GET THE TRANSACTION INDEX - ADD THIS LINE
                      ...
                      Tx.findOne({txid: txid}).then((tx) => {
                        if (tx && check_only != 2) {
                          ...
                          check_delete_tx(tx, block_height, timeout, function(updated_txes, tx_deleted) {
                            ...
                            if (tx_deleted || !tx) {
                              // save the transaction to local database - PASS txIndex parameter
                              module.exports.save_tx(txid, block_height, block, txIndex, function(err, tx_has_vout, newTx) {
                                if (err) {
