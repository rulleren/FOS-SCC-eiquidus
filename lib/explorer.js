...
module.exports = {
  ...
  get_geo_location: function(address, cb) {
    request({uri: 'http://ip-api.com/json/' + address, json: true}, function (error, response, geo) {
      if (error) {
        console.log('Geolocation API error for ' + address + ':', error);
        return cb(null, { country_name: 'Unknown', country_code: 'XX' });
      }

      if (!geo || geo.status === 'fail') {
        console.log('Geolocation lookup failed for ' + address);
        return cb(null, { country_name: 'Unknown', country_code: 'XX' });
      }
